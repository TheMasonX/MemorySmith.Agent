    ...
    IReplanGovernor? replanGovernor = null) : BackgroundService
{
    ...
    private void TryInterruptOnDamage(DamageTakenEvent damage)
    {
        ...
        var elapsed = DateTimeOffset.UtcNow - _lastDamageInterruptAt;
        ...
        _journal?.Log(new JournalEntry(
            DateTimeOffset.UtcNow, JournalEntryType.ActionFailed, "DamageInterrupt",
            new Dictionary<string, object?>
            {
            ...
            if (action is not null)
            {
                ...
                try
                {
                    ...
                    var pending = new PendingAction(correlationId, action.Tool,
                        DateTimeOffset.UtcNow, ActionLifecycle.Dispatched);
    ...
    {
        var cutoff = DateTimeOffset.UtcNow.AddSeconds(-DefaultActionTimeoutSeconds);
        foreach (var kv in _correlatedActions)
        {
            if (kv.Value.State == ActionLifecycle.Dispatched && kv.Value.DispatchedAt < cutoff)
            {
                ...
                if (_correlatedActions.TryUpdate(kv.Key, timedOut, kv.Value))
                {
                    logger.LogWarning(
                        ...
                        (DateTimeOffset.UtcNow - kv.Value.DispatchedAt).TotalSeconds);
                }
